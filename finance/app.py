...
def buy():
    ...
    if request.method == "GET":
        ...
        if user_cash < transaction_value:
            return apology("Insufficient ballance")
...
def deposit():
    ...
    if request.method == "GET":
        ...
        if  not amount:
            return apology("Please Insert Amount to Continue")

        user_id = session["user_id"]
        user_cash_db = db.execute("SELECT cash FROM users WHERE id = :id", id= user_id)
        user_cash = user_cash_db[0]["cash"]

        uptd_cash = user_cash + amount
...
def register():
    ...
    if request.method == "GET":
        ...
        if not username:
            return apology("Please type Username")

        if not password:
            return apology("Please type Password")

        if not confirmation:
            return apology("Please type Confirmation")
